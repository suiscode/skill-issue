...
export class AuthService {
  ...
  async signUp(input: SignUpInput): Promise<AuthPayloadType> {
    ...
    const { data, error } = await this.supabaseAuthService.publicClient.auth.signUp({
      email: input.email,
    ...
    try {
      ...
      await this.supabaseAuthService.adminClient.auth.admin.deleteUser(data.user.id);
      throw new InternalServerErrorException('Failed to create user profile');
    }
  }

  async signIn(input: SignInInput): Promise<AuthPayloadType> {
    const { data, error } = await this.supabaseAuthService.publicClient.auth.signInWithPassword({
      email: input.email,
      password: input.password,
    });
